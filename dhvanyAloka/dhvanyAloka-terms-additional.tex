...
\usepackage{fancyhdr}
\usepackage[style=iso]{datetime2} % Add this line

\pagestyle{fancy}
\fancyhf{} % clear all existing headers and footers
\fancyfoot[L]{DRAFT \today} % Left footer
\fancyfoot[C]{\thepage}     % Center footer
\renewcommand{\headrulewidth}{0pt}

% Ensure the footer also appears on the title page (which uses 'plain' style)
...
% Don't print page numbers if some overall page number is added
...
\begin{description}
	...
	% TODO: add MW def, at least the 1 most relevant?
	\item[K] \textit{\textbf{K}ārikā} (verse) of the \textit{Dhvanyāloka} (`Light on Suggestion'),
		...
		% kārikā
		% (H2) [Printed book page 274,3]
		% kārikā f. (f. of kāraka), a female dancer, L. [ID=48583]
		% a business, L. [ID=48583.1]
		% trade, L. [ID=48583.2]
		% concise statement in verse of (esp. philos. and gramm.) doctrines, MBh. ii, 453 &c. [ID=48583.3]
		% torment, torture, L. [ID=48583.4]
		% interest, L. [ID=48583.5]
		% N. of a plant, L. [ID=48583.6]
		% f. pl. or more commonly hari-kārikās, the Kārikās of Bhartṛ-hari i.e. the verses contained in his gramm. work Vākya-padīya (q.v.) [ID=48583.7]

	% TODO: maybe reword this so it makes more obvious sense that guy wrote K and A at same time? Or was there a time gap??
	% TODO: ?? add something like I would have expected 'V' for \textit{vṛtti}, but I suppose the translators chose `A' for Ānandavardhana. But `Ā' would have been clearer, as we get `[A]' to denote Abhinavagupta's responses to the Mīmāṃsaka (`[M]') position in 2.4 L
	...
	% locana
	% (H2) [Printed book page 907,3]
	% locana mfn. illuminating, brightening, BhP. [ID=183580]
	% locana m. N. of an author, Cat. [ID=183581]
	% locana f(ā or ī). N. of a Buddhist goddess, Dharmas. 4 [ID=183582]
	% (H2B) [Printed book page 907,3]
	% locanī f. a species of plant, L. [ID=183583]
	% locana n. (ifc. f(ā). ) ‘organ of sight’, the eye, MBh. ; Kāv. &c. [ID=183584]
	% N. of wk. [ID=183585]
	% Whitney's Roots: √loc 'to see, to consider'
	%ध्वन्यालोक -लोचनम्‌ -> dvhanyAloka-locanam -> is locanam accusative singular or adverbial?? or h. to m for sandhi??
\end{description}
...
\begin{description}
	...
	\item[Bhaṭṭanāyaka] critic of Ānandavardhana, apparently a Mīmāṃsaka, answered by Abhinavagupta. The translators will use the abbreviation `BN' inside their in-line bracketed additions.
	      ...
	      % TODO: who is meant by `BN' in the translator's brackets?? seen in 2.4 L, EC p.135
\end{description}

\subsection{Sections of text included in EC Reading}

% NOTE: one line break is not counted as a new paragraph, all below is typeset/rendered as one continuous line
Translator's Introduction (15--17);
\textbf{1.1} K, A, L (47--49 partial);
% TODO: seperate out L and make clear the middle is skipped?
\textit{--skipping--};
...
Notation follows section numbering in IMP translation. Page numbers are given when translation of K. Krishnamoorthy is used.
...
% TODO: write out gunas here??
% TODO: see notes in comments below...
% TODO: check `marvelous' is right for 8th rasa - from 2.9 L i am pretty sure.
% TODO: for wording on peace's guṇa, see EC p. 151
...
% 		IMP, pg16: BhNS 6.31 +3 (the famous rasasütra) tells us th at “A rasa is pro­
% duced by the combining of the determ inants (vibhävas), the conse­
% quents (anubhävas), and the tem porary or transient states of mind
...
% disgust.
		...
		\litem{anubhāva} ``consequent'' (IMP Intro. pg 16),
		``symptoms'' (1.4b L)

		% TODO: check 2.3 K reference..., none earlier??
		% TODO: is emotion from the IMP text itself, or into?
		\litem{bhāva} ``emotion'' (1.4g L, 2.3 K) [K. Kris. Also B. Perry's translation, from IMP's Introduction];
		``realization'' (1.1e L, quote from \textit{BhNŚ})
		...
		\litem{rasa} ``flavor'', ``sentiment'' (TODO REF, IMP Intro?)
		 --- each rasa is connected to one \textit{sthāyibhāva} (term \#\ref{sthāyibhāva}), as shown in the table above.
		...
		\litem{vibhāva} ``determinant''  (IMP Intro. pg 16, 1.18 L.1),
		``stimulating determinant'' (1.4b L);
		``object'' (1.1e L, quote from \textit{BhNŚ})
		\begin{enumerate}
			%\setlength{\itemsep}{-0.25em}
			% TODO: add reference
			\litem{ālambanavibhāva} ``objective determinant'' %(IMP Intro. pg 16)

			% TODO: add reference
			\litem{uddīpanavibhāva} ``stimulative determinant'' %(IMP Intro. pg 16)
		\end{enumerate}

		% TODO: state... of mind??? check REF 
		\litem{vyabhicāribhāva} ``transitory state'' (1.5 L), 
		``transient state of mind'' (2.3 L), %maybe also (1.4g L)??
		``transient emotion'' (1.18 L, 2.3 L),
		e.g. ``shyness'' (1.5 L)%(IMP 117)

		% TODO: add reference
		% TODO: which of the two vyabhic... is more used? make that the primary def/label
		% TODO: move under the primary usage/word?
		...
		% TODO: add reference - used in 1.4g L? but no def?? not sure what my notebook note means...
		% TODO: check ``stable emotion'' EC p.133
		\litem{sthāyibhāva} ``abiding emotion''  (TODO REF, IMP Intro?),
		``basic emotion'' (1.5 A \& L, 1.18 L),
		e.g. ``grief (\textit{śoka}) is the basic emotion of the flavor of compassion (\textit{karuṇarasa})'' (1.5 A), %(IMP 114), also in K. Kris. as: ``Sorrow indeed is the abiding emotion'' (p13), sanskrit is: ``śoko hi karuṇasthāyibhāvaḥ'' (p12, line 10)
		``stable emotion'' (2.4 L)

	\end{enumerate}

	% \vspace{-2ex} % no good answer here to make column breaks more readable
	...
	\begin{enumerate}[
		...
		\setlength{\itemsep}{0.15em}
		\setcounter{enumi}{7}% set to number of last item above.

		% TODO: move to new column? rearrange so it isn't split over 2.
		\litem{dhvani} ``suggestion'' (1.1 K, 1.1 A)
		Also ``suggested meaning'' (1.4g A).
		The three types of \textit{dhvani} per 1.4a A are:
		\begin{enumerate}

			\litem{vastudhvani} ``the suggestion of a fact'' (1.1e L, 1.4a A), %(IMP 70, 1.1e L)
			...
			``the suggestion is produced\dots where the sequence is apparent'' (1.4g L). %IMP p106
			From:
			\begin{itemize}
			% TODO: maybe wrong? see 1.1 K.2, EC p.77. Connected with bhākta?
			% 2. Änanda uses the word bhâkta to cover both types of
			...
			% A By wise men, that is, by those who know
				\litem{alaṅkāra} ``figures of speech'' (1.1 L, 1.1a A, 1.4g L),
			...
			\textit{bhāvābhāsa}, % page ?, 1.4g L IMP pg 107
			...
			\begin{itemize}
				...
				Explained as: ``The term refers to all elements that belong to \textit{rasadhvani}'' (1.4a A, defined 2.5 L) % EC p.143?
				NB: \textit{ādi}: ``etcetera'' (2.3 L)%p.216 orig.
				
				
				% TODO: delete? moved this to 1st def block.
				%\item \textit{rasadhvani} is the ``soul of poetry'' (1.5 K)

				% TODO: keep this?? extra/doesn;t add anything?? ALSO CHECK SAME NOW WITH ADDTION OF [notes]
		...
		% TODO: to wordy?? not enough useful info??
		\litem{vikalpa} ``alternative idea'' (1.1 L),
		...
		% Commented out for now, prob not useful
		% \litem{------1} ``outside the scope of speech''? (1.1 K) TODO: a term, or just often used phrase?



		% TODO: add more details, or the fuller context of a sentence here??
		\litem{guṇa} the ``qualities'' (1.1 L, 1.1a A, 2.7 K \& A), ``virtues'' (2.7 K.1)
		\begin{enumerate}
			%\setlength{\itemsep}{-0.25em}
			% TODO: use above line about sweetness has its seat ???
			\litem{mādhurya} ``sweetness'' (2.7 K), % IMP pg 252
		...
		% TODO: add MW def?? ha some of it at least?
		...
		% vṛtti
		% (H2) [Printed book page 1010,1]
		% vṛtti f. rolling, rolling down (of tears), Śak. iv, 5; 14 [ID=204501]
		% mode of life or conduct, course of action, behaviour, (esp.) moral conduct, kind or respectful behaviour or treatment (also v.l. for vṛtta), GṛŚrS. ; Mn. ; MBh. &c. [ID=204502]
		...
		% profession, maintenance, subsistence, livelihood (often ifc.; cf. uñcha-v°; vṛttiṃ-√kṛ or √kḷp [Caus.] with instr., ‘to live on or by’; with gen., ‘to get or procure a maintenance for’; only certain means of subsistence are allowed to a Brāhman See Mn. iv, 4-6 ), ŚrS. ; Mn. ; MBh. &c. [ID=204508]
		...
		% mode or measure of pronunciation and recitation (said to be threefold, viz. vilambitā, madhyamā, and drutā q.v.), Prāt. [ID=204513]
		...
		% (in rhet.) alliteration, frequent repetition of the same consonant (five kinds enumerated, scil. madhurā, prauḍhā, puruṣā, lalitā, and bhadrā), Daśar., Introd. [ID=204516]
		% final rhythm of a verse (= or v.l. for vṛtta q.v.) [ID=204517]
		...
		\litem{guṇavṛtti} TODO: Fix up this section! ``secondary usage'' (1.18 K), ``indication'' [K. Kris.\ p.35],
		...
		\begin{itemize}
			...
			% TODO: take out from under main gunavrtti??
			% TODO: see footnote 1.1 K.2 - bhākta is used closely with guṇavṛtti
		...
		\litem{rīti} ``style'' (1.1a A, 1.1a L) % IMP pg 54, 56
		% also (1.1a L.8)?? also vṛttikāra (1.1e L)??

		\litem{sahṛdayāṇām} ``of sensitive readers'' \{genitive plural of term \#\ref{sahṛdaya}, `\textit{sahṛdaya}'\} (1.1e A, 1.1e L, also 1.1 L.1)

		% TODO: why did i make 'with' italic?
		\litem{sahṛdaya} ``having their hearts \textit{with} it'' (1.1e L);
		from `with/junction' \textit{sa-} + `heart' \textit{hṛdaya},
		``with the heart; hearty, sincere; possessing a heart, good-hearted, full of feeling, sensible, intelligent; a learned man'' (MW 1195)
		% sahṛdaya
		% (H3) [Printed book page 1195,2]
		% sa—hṛdaya mf(ā)n. (sa-) with the heart, TBr. ; Vikr. [ID=240480]
		% hearty, sincere, AV. [ID=240480.1]
		% possessing a heart, good-hearted, full of feeling, sensible, intelligent, Kāv. ; Daśar. ; Sāh. [ID=240480.2]
		% sa—hṛdaya m. a learned man, W. [ID=240480.3]
		% sa—hṛdaya mn. (?) N. of wk. on Ācāra [ID=240480.4]
		...
		% TODO: verify k kris def. include MW?
		\litem{prasiddha} 1.\ ``well known to all'' and ``ornamented'' (1.4 L),
		% \{used in 1.4 K: ``well-known elements [of poetry]'' \& in 1.4 A:\ ``known, ornamented, elements [of poetry]''\},
		``striking'' external constituents [K. Kris.\ p.7, 1.4 K],
		%\{Further, ``The expression `striking' connotes not only what is `adorned with figures' but also what is `perceptible to the senses'.''\}
		``The expression `striking' connotes not only what is `adorned with figures' but also what is `perceptible to the senses'.'' [K. Kris.\ p.9, 1.4 A]
		% prasiddha
		% (H3) [Printed book page 697,3]
		% pra-°siddha mfn. (pra-) brought about, accomplished, Kum. (a-pras°) [ID=138003]
		% arranged, adorned (as hair), ib. [ID=138003.1]
		% well known, notorious, celebrated, TS. &c. &c. [ID=138003.2]
		% (H3B) [Printed book page 697,3]
		% pra-°siddhā f. (in music) a partic. measure, Saṃgīt. [ID=138003.3]
		...
		\litem{bhāvapraśānti} ``(rise and) cessation'' of emotion (or, `mood') [\textit{bhāva}, term \#~\ref{bhāva}] (2.3 K) Also see ninth rasa, \textit{śānta} (`peace')
		% praśānti
		...
		\litem{rasavat} ``that which contains rasa in a subordinated position'' (2.3 L),
		Ānanda  assigns ``rasadhvani to cases where the rasa is predominant and limiting the figure of speech [rasavat] to cases where the rasa is subordinate or ornamental.''
		(2.4 Intro. A.1) %IMP pg 220
		%``ornamental'' (2.4 Intro. A.1), % TODO: useful??
		%ALSO:
		%\litem{rasavadalaṅkāra} ``figurative sentiment'' (2.4 Intro. A) [K. Kris.\ p.41] % IMP pg 219 % JKL Note: i.e. rasavat + alankara, sandhi t -> d
		%See term \#~\ref{rasavat}
		% Useful?
		% ``Daṇḍin says merely that it was a figure charming with \textit{rasa} (2.4 Intro. A.1) (Daṇḍin?: 2.275).
		...
		% % TODO: where is this from -> Note: ``found alike in force [and sweetness]'' is it a K. Kris. translation?? but the [...] makes me think not...
		% %       Is it from 2.7 A: "For, sound-harmony is found alike in forcefulness too (and is not a differentia of sweetness)." ??
		% % TODO: where from?
		% % TODO: get term or definition
		% \litem{pleasing to the ear} (2.7 A), % JKL Note: part of sweetness madhurya, but not all of it, as 'pleasing to the ear'/sound-harmony is found in forcefullness too (not just srngara)
		...
		% TODO: get def w/o ???
		\litem{racanā} ``structures'' (2.10 A), % IMP pg 259
		``composition'' [K. Kris.\ p. TODO] % TODO: get K. Kris. pg num
		...
		% TODO: some note about this is what it means in this context, but has more general meaning of... MW??
		% TODO: give sound meaning from K Kris. !!!! and then ref that karika or vrtti as well from IMP?
		% TODO: give MW def, refer to alankara term above \ref{alaṅkāra}
		\litem{śabdaḥ} (a): ``a word which gives rise to suggestion'' (1.13 L)

		% TODO: some note about this is what it means in this context, but has more general meaning of... MW?
		% TODO: give MW def, refer to alankara term above \ref{alaṅkāra}
		\litem{arthaḥ} (b) ``a meaning which gives rise to a suggestion'' (1.1 K.1, 1.13 L)%(IMP 47, 1.1 K.1) which refers to (IMP ???, 1.13 L)
		...
		% TODO: keep? delete? context more?
		\litem{vācyatva} ``being denoted'' (1.4g A.1)
		...
		%TODO: get reference to that Note:...
		\litem{samaya}  ``conventions by which words transmit meaning'' (1.1 L)
		\{Note: this used in reference to an alternative idea (see term \#~\ref{vikalpa}.\ \textit{vikalpa}) against the concept of dhvani, argued against in the Dhvanyāloka\}
		...
		\litem{lakṣayatām} those ``who are noticing'' (1.1e A);
		``describing it by means of a definition'' (1.1e L).
		Possibly used differently by Ānandavardhana and Abhinavagupta, see: 1.1e A.2.

		% TODO: check `delight' translation
		\litem{ānanda} ``bliss'', ``bliss, which is nothing more than the relishing of \textit{rasa}'' (1.1e L),
		``delight''? (1.1 K),
		also name of the author \{Ānandavardhana\} (1.1e L)
		...
		%TODO: somehow better group these the 3 parts of poetry according to Bhaṭṭanāyaka, 1.1e L EC p.87? or better say where these still are agreed with or not by DhvanyAloka??
		\litem{rasacarvaṇā} ``relishing of \textit{rasa}'' \{one of the ``three parts'' which ``poetry consists of'' according to Bhaṭṭanāyaka\} (1.1e L),
		described as ``it is the relishing of \textit{rasa} that gives [poetry] its life'' (1.1e L)

		\litem{rasanā} ``Relishing (\textit{rasanā}) is a special kind of perception. This perception [of \textit{rasa}] in drama is distinct from every-day cases of intererence\dots'' (2.4 L)
		% TODO: EC p134, why the difference? is it about relishing in drama, not poetry? or just a case ending/grammar diff etc?
		...
		\litem{bhogakṛttva} From Bhaṭṭanāyaka: ``efficacy of aesthetic enjoyment'' or ``the power of aesthetic enjoyment'' (2.4 L) 
		% TODO: use???
		% TODO: how connected with āsvāda??
		%EC p.131-132, orig p.221-222

		\litem{bhogīkaraṇa} ``[What Bhaṭṭanāyaka calls] the poem's operation of causing aesthetic enjoyment (\textit{bhogīkaraṇa}) of the \textit{rasa}s is nothing other than the operation of suggestiveness.'' (2.4 L)
		%EC p.135, orig p.225

		\litem{druti} ``melting of the mind''? \{a form of relishing/aesthetic enjoyment\} (1.5 L.3, 2.4 L)
		% TODO: i have a note about ``Which takes the form of...'' 2.4 L IMP pg 222 line 15, also note 15 on IMP pg 228: 
		% 15. Druti, vistara, and vikäsa. We can point to no technical use of these terms earlier than
		...
		% the rasas, see Gnoli, p. 46, and Raghavan, Bhoja, pp. 467ff.

		\litem{vistara} ``expansion'' \{a form of relishing/aesthetic enjoyment\} (1.5 L.3, 2.4 L)

		\litem{vikāsa} ``radiance'' \{a form of relishing/aesthetic enjoyment\} (1.5 L.3, 2.4 L),
		``expansion'' (2.9 L)
		% TODO: check duplicate use of `expansion'...
		% TODO: group these three under āsvāda or something??
		% TODO: ?? use my note of: ``varieties of relishing ... are innumerable'' - not just these three - EC p.136


		% TODO 'great poet' 1.4 L add def or sanskrit word? not sure...


		\litem{rāga} ``passion'' (2.8 L) %IMP pg 254

		\litem{anurāga} ``stimulation'' (1.4a L)

		\litem{abhidhā} ``designation'' \{one of the ``three parts'' which ``poetry consists of'' according to Bhaṭṭanāyaka\} (1.4a L),
		``denotation'' (1.5 L),
		``direct denotation'' (2.4 L); % IMP pg 224
		``name, appellation'' or
		...
		\litem{bhāvanā} ``aesthetic efficacy'' \{one of the ``three parts'' which ``poetry consists of'' according to Bhaṭṭanāyaka\} (1.4a L),
		...
		% TODO: add MW def for citta as a way to compare to gun.avr.tti???
		\litem{cittavṛtti} ``state of mind'' (1.4g L),
		``thought-trend'' TODO: is this disproved later in text? (1.5 L), % TODO: is this confusing or disproved? see (later on in text, ugh i forget where exactly)...
		...
		% cittavṛtti
		% (H3) [Printed book page 396,1]
		% citta—vṛtti f. state of mind, feeling, emotion, Śak. ; Pañcat. ; Ṛtus. ; Kathās. [ID=73593]
		% continuous course of thoughts (opposed to concentration), thinking, imagining, Yogas. i, 2 ; Bhar. ; Naiṣ. viii, 47 ; Sarvad. ; Hit. [ID=73594]
		% disposition of soul, Vedântas. [ID=73595]
		% FROM FROM FROM:
		% citta
		% (H2) [Printed book page 395,3]
		% citta a mfn. ‘noticed’ See a-citta [ID=73536]
		% ‘aimed at’, longed for, ChUp. vii, 5, 3 [ID=73537]
		% ‘appeared’, visible, RV. ix, 65, 12 [ID=73538]
		% citta n. attending, observing (tiraś cittāni, ‘so as to remain unnoticed’), , vii, 59, 8 [ID=73539]
		% thinking, reflecting, imagining, thought, RV. ; VS. ; ŚBr. &c. [ID=73540]
		% intention, aim, wish, RV. ; VS. ; AV. ; TBr. &c. [ID=73541]
		% citta n. (Naigh. iii, 9 ) the heart, mind, TS. i ; ŚvetUp. vi, 5 ; MBh. &c. (ifc. f(ā). , Pañcat. ) [ID=73542]
		% citta n. memory, W. [ID=73543]
		% intelligence, reason, KapS. i, 59 ; Yogas. i, 37; ii, 54 ; Vedântas. [ID=73544]
		% (in astrol.) the 9th mansion, VarYogay. iv, 1 [ID=73545]
		% cf. iha-, cala-, pūrva-, prāyaś-, laghu-, su-, sthira-. [ID=73546]
		% (H1) [Printed book page 398,1]
		% citta b See √4. cit. [ID=74097]
		...
		% TODO: delete or keep?
		% TODO: is there an easy to make connection to artha ?? is this `self-meaning' or `directly denoted'??
		...
		% TODO: delete or keep?
		% TODO: also 'power'? dhvananaśakti ``power of suggestion'' (IMP, General Index, p794)
		\litem{śakti} ``power''?, ``force''?: ``When the suggestion of \textit{rasa} is ascribed to the directly denoted meaning \dots
		...
		% \litem{gocara} ``direct object'' (2.4 L) % TODO: delete??
		...
		\litem{abhāvavāda} ``non-existence'' - the third alternative idea (vikalpa, term \#\ref{vikalpa}) (1.1e A.1)

		\litem{bhāktavavāda} ``dependent'' - the third alternative idea (vikalpa, term \#\ref{vikalpa}) (1.1e A.1)

		\litem{anākhyeyavāda} ``not telling?'' argument - the third alternative idea (vikalpa, term \#\ref{vikalpa}) TODO or anākhyāyavāda?(1.1e A.1)
		% TODO expand on these. EC p85
		% TODO find replace \ref with \#\res

		% TODO: add this in some way??
		% ślokah. śokatvam āgatah. - 1.5 K.1, but in K? (IMP 113),
		% connected to agamah. - (1.5 L, IMP 116) ??

		% \litem{vipralambhaśṛṅgāra} ``erotic flavor based on the seperation of lovers'' (1.5 L.1)%(IMP 118)
		% ``the \textit{rasa} of love in separation''?? this isn't right translation... (2.8 K?)

		% TODO: is this correct? is this useful?
		% \litem{nirapeks. abhāva} ``the hopelessness of tragic grief''?? (1.5 L.1)%(IMP 118)
...
% see also bhävälatikära, sthāyibhāva,
% sättvikabhäva.
...
% n7, example 215.
% bhaya (fear), the sthāyibhāva of bhayä-
% nakarasa 507 n7.
