...
\begin{multicols}{2}
	...
	\begin{enumerate}[
		...
		\litem{anubhāva} ``consequent'' (TODO REF, IMP Intro?)
		...
		\litem{rasa} ``flavor'', ``sentiment'' (TODO REF, IMP Intro?) --- based on the \textit{sthāyibhāva}s (term \#\ref{sthāyibhāva}) respectively, as shown in the table above.
		...
		\litem{vibhāva} ``determinant'' (TODO REF, IMP Intro?); ``object'' (1.1e L, quote from \textit{BhNŚ})
		\begin{enumerate}
			%\setlength{\itemsep}{-0.25em}
			% TODO: add reference
	...
	\begin{enumerate}[
		...
		\litem{dhvani} ``suggestion'' (1.1 K, 1.1 A) Also ``suggested meaning'' (1.4g A).
		...
		% TODO: where from?
		% TODO: get term or definition
		\litem{pleasing to the ear} (2.7 A),
		...
		\litem{samaya}  ``conventions by which words transmit meaning'' (1.1 L) \{Note: this used in reference to an alternative idea (see term \#~\ref{vikalpa}.\ \textit{vikalpa}) against the concept of dhvani, argued against in the Dhvanyāloka\}
		...
		\litem{ānanda} ``bliss'', ``bliss, which is nothing more than the relishing of \textit{rasa}'' (1.1e L), ``delight''? (1.1 K), also name of the author \{Ānandavardhana\} (1.1e L)
		...
		\litem{bhogakṛttva} From Bhaṭṭanāyaka: ``efficacy of aesthetic enjoyment'' or ``the power of aesthetic enjoyment'' (2.4 L) % TODO: use???
		...
		% TODO: i have a note about ``Which takes the form of...'' 2.4 L, but not sure where that quote is...
		...
		``thought-trend'' (1.5 L), % TODO: is this confusing or disproved? see (later on in text, ugh i forget where exactly)...
		...
		% TODO: keep delete? place under anuraga?
