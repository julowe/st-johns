...
% \usepackage{cm-unicode} % not installed by default with texworks at least
\usepackage{fontspec}
...
\DTMsavedate{fall-start}{2024-09-02}
\DTMsavedate{spring-start}{2025-01-20}
\DTMsavedate{summer-start}{2025-06-16}
\DTMsavedate{fall-precept1-start}{2024-09-03}
\DTMsavedate{fall-precept2-start}{2024-10-29}
\DTMsavedate{summer-precept-start}{2025-06-16}

%date for break are inclusive, i.e. there are no classes on start or end dates
\DTMsavedate{break-start-thanksgiving}{2024-11-27}
\DTMsavedate{break-end-thanksgiving}{2024-12-01}
\DTMsavedate{break-start-spring}{2025-03-15}
\DTMsavedate{break-end-spring}{2025-03-30}
...
% italicize titles
% copy in text with diacritics etc

\usepackage[calc,useregional]{datetime2}

% TODO:
% function - \getNextBiweeklyClassDate: just take DoW of date and advance it to next specified day of a class
% function - \checkIfHoliday - check if date is betwen holidays, if so return last day of holidays+1 - pass output to get next class day
...
\newcommand{\printHeaderForNextSeminarClassDate}[1]{%
  %TODO: make checkIfHoliday function, and use it here.
...
\newcommand{\printHeaderForNextPreceptClassDate}[1]{%
  %TODO: make checkIfHoliday function, and use it here.
 %test%\DTMusedate{currentClassDate}%
  \getNextBiweeklyClassDate{1}{3}{#1}%
}

% TODO: make function to check if session is in holidays?
% if date is between the two, then return break-end... + 1, else return date
%
...
% e.g. :
%    \DTMsavedate{fall-start}{2024-09-02} % Note: a monday
%    Usage: type in document `\getNextBiweeklyClassDate{0}{3}{fall-start}`
%        Notes: 0 and 3 indicate classes are Monday (0), and Thursday (3).
...
\newcommand{\getNextBiweeklyClassDate}[3]{%
  ...
  \ifnum \DTMfetchdow{#3}<\value{DoWfirst}%
    ...
    \DTMsavejulianday{currentClassDate}{\number\myNextClassRegister}%
  \else%
  ...
  \printHeaderWithSessionAndDate{currentClassDate}%
}


%% function - \printHeaderForFirstSeminarClassDate: helper function that takes in a specified date, saves it to `currentClassDate`, then just runs \printHeaderWithSessionAndDate - yeah this is an ugly workaround to the function flow I already started coding, but it's what you get at this late hour.
%\newcommand{\printHeaderForFirstSeminarClassDate}[1]{%
%  \DTMsavedate{currentClassDate}{#1}
%}

\newcount\myLastPrintedClassRegister
%function - \printHeaderWithSessionAndDate: increments cntSession (since it starts at 0), then prints text for Session <cntSession> - <currentClassDate> 
\newcommand{\printHeaderWithSessionAndDate}[1]{%
	\stepcounter{cntSession}%
	\subsection{Session \arabic{cntSession} - \DTMusedate{#1}}%

  \DTMsaveddateoffsettojulianday{#1}{0}{\myLastPrintedClassRegister}%
  \DTMsavejulianday{lastPrintedClassDate}{\number\myLastPrintedClassRegister}%
  %These don't work, hence doing the dumb julian dance
  %\DTMsavedate{lastPrintedClassDate}{#1}
  %\DTMsavedate{lastPrintedClassDate}{\DTMusedate{#1}}
}
...
% TODO: make skip session function - advance cntClassOffset (& cntWeek if needed)
...
\begin{multicols}{2}
	...
	\printHeaderWithSessionAndDate{fall-start}
	...
	\begin{comment}
	Session 1Session 8
	Confucius, Analects, \#1-7. Skip commentary
	where present.Xunzi (Hsun Tzu), Sections 1, 2, 9, 17. Use
	translations by Burton Watson or Eric L.
	Hutton.
	Session 2
	Confucius, Analects, \#8-13. Skip commentary
	where present.Session 9
	Session 3Session 10
	Xunzi (Hsun Tzu), Sections 19-23.
	Confucius, Analects, \#14-20. Skip
	commentary where present.
	Zhuangzi (Chuang Tzu), Chapters 1-3
	Session 11
	Session 4
	Mozi (Mo Tzu), Fascicles 11, 14-17, 19, 20, 26,
	27, 31, 32, 35, 39, and 50. Translations by
	Burton Watson (Mo Tzu Basic Writings), Ian
	Johnston (The Book of Master Mo) or Chris
	Fraser (The Essential Mozi) are acceptable.
	Session 5
	Mengzi (Meng Tzu, Mencius), Books I – II (1A, 1B,
	2A, 2B).
	Session 6
	Zhuangzi (Chuang Tzu), Chapters 4-7
	Session 12
	...
	Chapters 1-37. Commentaries are not
	necessary.
	Session 14
	Mengzi (Meng Tzu, Mencius), Books III-IV (3A,
	3B, 4A, 4B).Laozi (Lao Tzu), Dao De Jing (Tao Te Ching),
	Chapters 38-81. Commentaries are not
	necessary.
	Session 7Session 15
	Mengzi (Meng Tzu, Mencius), Books V -VII (5A,
	5B, 6A, 6B, 7A, 7B).
	Han Feizi (Han Fei Tzu), Fascicles 20, 21.
	Available in EC Manual.
...
\begin{multicols}{2}
	...
	\printHeaderWithSessionAndDate{spring-start}
	...
	\printHeaderForNextSeminarClassDate{lastPrintedClassDate}
	%FIXME: m dot is not printing well... 
	...
	``Pūrva Mīmāṁsā'' in \emph{A Sourcebook in Indian Philosophy} edited by Sarvepalli Radhakrishnan and Charles A. Moore\emph{.} Pp. 486 -- 505.

	\printHeaderForNextSeminarClassDate{lastPrintedClassDate}
	``Cārvāka'' in \emph{A Sourcebook in Indian Philosophy.} Pp. 227 -- 249.

	\printHeaderForNextSeminarClassDate{lastPrintedClassDate}
	``Discourses on the Nobel Quest,'' ``Discourse to Kālāmas,'' and ``The Greater Discourse on Cause'' from \emph{Early Buddhist Discourses} by John Holder\textbf{.}
	...
	\printHeaderForNextSeminarClassDate{lastPrintedClassDate}
	Zhu Xi (Chu Hsi), ``The Complete Works of Chu Hsi'' in \emph{A Sourcebook in Chinese Philosophy}, pp. 632-653, 693-604\emph{.}
	...
	Session 1
	Kālidāsa, Kumārasaṃbhava, in The Origin of the
	Young God, translated by Hank Hifetz.
	Available in EC Manual.
	Session 2
	Kālidāsa, The Recognition of Śhakuntalā,
	translated by W.J. Johnson.
	Session 3
	“The Dhvanyāloka of Ānandavardhana with
	the Locana of Abhinavagupta” selections with
	supplemental material by Keith and Perry.
	Through pp. 119. Available in EC Manual.
	Session 4
	“The Dhvanyāloka of Ānandavardhana with
	the Locana of Abhinavagupta” selections with
	supplemental material by Keith and Perry.
	Sections through pp. 696. Available in EC
	Manual.
	Session 5
	“Pūrva Mīmāṁsā” in A Sourcebook in Indian
	Philosophy edited by Sarvepalli
	Radhakrishnan and Charles A. Moore.
	Pp. 486 – 505.
	Session 6
	“Cārvāka” in A Sourcebook in Indian
	Philosophy. Pp. 227 – 249.
	Session 7
	“Discourses on the Nobel Quest,” “Discourse
	to Kālāmas,” and “The Greater Discourse on
	Cause” from Early Buddhist Discourses by
	John Holder.
	Session 8
	“The Greater Discourse on the Foundations of
	Mindfulness,” “The Greater Discourse on the
	Destruction of Craving,” “Discourse of the
	Honeyball” from Early Buddhist Discourses.
	Session 9
	“Short Discourses from the Saṃyutta Nikāya,”
	“The Shorter Discourse to Māluṅ kyaputta,”
	“Discourse on the Parable of the Water
	Snake,” “Discourse to Vacchagotta on Fire,”
	from Early Buddhist Discourses.
	Session 10
	“Discourse to Prince Abhaya,” “Discourse to
	Poṭṭhapāda,” “Discourse on the Threefold
	Knowledge,” from Early Buddhist Discourses.
	Session 11
	“Discourse to Assalāyana,” “Discourse to the
	Layman Sigāla,” from Early Buddhist
	Discourses. Samannaphala Sutta (“Fruits of
	the Homeless Life”), available in EC Manual.
	Session 12
	The Lotus Sutra, Translated by Burton Watson.
	Chapters 1 (prose portion), 2-7 (only the parable
	in the last nine paragraphs of the final prose section,
	beginning with “Monks, you must understand this.”),
	and 8.
	Session 13
	The Lotus Sutra, Translated by Burton Watson.
	Chapters 10 (prose portion), 11 (verse only),
	12-14, 16, 20, 21, 23, 25, 28 (all prose
	portion).
	P a g e |4EASTERN CLASSICS
	SEMINAR
	SPRING - SEMINAR
	(Continued)
	Session 14
	Session 20
	Nāgārjuna, Chapters 1, 2, 7.
	Saṅ karâkârya’s commentary on the Vedânta
	Sūtras. pp. 2 – 67, Sacred Books of the East, Vol.
	34, translated by George Thibaut. Available as
	a photocopy in the bookstore. In reading Sankara
	Translation options:
	(A)Mūlamadhyamakakārikā of Nāgārjuna:
	The Fundamental Wisdom of the Middle Way,
	translated by Jay L. Garfield OR
	(B)The Philosophy of the Middle Way
	translation by David J. Kalupahana.
	and Rāmānujā pay close attention to whether you are
	hearing their own (Siddhanta) positions or their
	opponents’ (Purvapaksha) arguments. The latter can be
	lengthy and detailed (and plausible).
	Session 15
	Session 21
	Nāgārjuna, Chapters 10, 12, 17, 18
	Saṅkara, pp. 183 – 191, 199 – 216, 283 – 289,
	299 – 308, 312 – 320, 355 – 362.
	Session 16
	Nāgārjuna, Chapters 24, 25, 27
	Session 22
	Vimalakīrti Sūtra, The Holy Teaching of
	Vimalakīrti, translated by Thurman.
	Sections 1 – 6.Rāmānujā’s commentary on the Vedânta
	Sūtras, pp. 3 – 73, Sacred Books of the East, Vol.
	48, translated by George Thibaut. Available in
	EC Manual
	Session 18Session 23 [ESSAYS DUE]
	Session 17
	Vimalakīrti Sūtra, The Holy Teaching of
	Vimalakīrti, translated by Thurman.
	Sections 7 – End.
	Rāmānujā’s commentary on the Vedânta
	Sūtras, pp. 102 -119, 129 – 138, 255- 273,
	296- 308.
	Session 19
	Gauḍ apāda, “Māndukya Upanishad with the
	Kārikā of Gauḍ apāda” in Vol. II of The
	Upanishads translated by Swami Nikhilananda
	(Ramakrishna-Vivekananda Center), pp. 223
	– 368. Read both the Māndukya Upanishad and
	Gauḍ apāda commentary. Ignore other commentaries. In
	Chapter 1, after the Hari Om paragraph, read the
	paragraph following each heading numeral, following the
	woven threads of Upanishad and Karika (Gaudapada’s
	commentary). For example, “Now the Upanishad is
	resumed,” or “Now the Karika is resumed.” From Chapter
	2 on, the paragraph following the number is Gaudapada.
	Session 24
	Jayadeva’s “The Gītagovinda” in Love Song of the
	Dark Lord, edited and translated by Barbara
	Stoler Miller, pp. 69 – 125.
	Session 25
	“The Diamond Sūtra” in The Diamond Sūtra \&
	the Sūtra of Hui-Neng, translated by A.F. Price
	and Wong Mou-lam (Shambhala).
	“The Heart Sūtra” pp. 17 – 53. Available in EC
	Manual.
	P a g e |5EASTERN CLASSICS
	SEMINAR
	SPRING - SEMINAR
	(Continued)
	Session 26
	Hui-Neng, “Commentary on the Diamond
	Sūtra,” in The Sutra of Hui-neng, Grand Master of
	Zen: With Hui-neng's Commentary on the Diamond
	Sutra, translated by Thomas Cleary
	(Shambhala), pp. 85-144.
	Session 27
	Hui-Neng, The Platform Sutra of the Sixth
	Patriarch, translated by Philip Yampolsky
	(Columbia University Press), paragraphs 1 –
	30.
	Session 28
	Session 30
	Zhu Xi (Chu Hsi), “The Complete Works of
	Chu Hsi” in A Sourcebook in Chinese
	Philosophy, pp. 605 – 632 (stop at Jen).
	Session 31
	Zhu Xi (Chu Hsi), “The Complete Works of
	Chu Hsi” in A Sourcebook in Chinese
	Philosophy, pp. 632-653, 693-604.
	Session 32
	Wang Yang-Ming, “Inquiry on the Great
	Learning” and selections from “Instructions
	for Practical Living,” in A Sourcebook in
	Chinese Philosophy, pp. 659 – 691.
	Hui-Neng, The Platform Sutra of the Sixth
	Patriarch, 31 – 57.
	Session 29
	“The Great Learning” and “The Doctrine of the
	Mean,” in A Sourcebook in Chinese Philosophy,
	translated and edited by Wing-tsit Chan,
	pp. 84 – 114.
...
\begin{multicols}{2}
	...
	\printHeaderWithSessionAndDate{summer-start}
	...
	P a g e |6EASTERN CLASSICS
	SEMINAR
	SUMMER - SEMINAR
	Session 1
	Session 8
	The Tale of the Heike, translated by Helen Craig
	McCullough. Chapters 1 – 8.
	Dōgen, “Bendōwa,” in The Heart of Dōgen’s
	Shōbōgenzō, translated by Waddell and Abe
	(SUNY Press).
	Session 2
	The Tale of the Heike, Chapters 9 – 10.Session 9
	Session 3Dōgen, “Busshō,” in The Heart of Dōgen’s
	Shōbōgenzō, pp. 59 – bottom of 84.
	The Tale of the Heike, Chapters 11 – end.
	Session 10
	Session 4
	Dōgen, “Busshō,” in The Heart of Dōgen’s
	Shōbōgenzō, pp. 84 - 98.
	Kūkai Major Works translated by Yoshito S.
	Hakeda (Columbia University Press), “The
	Difference Between Exoteric and Esoteric
	Buddhism”, “Attaining Enlightenment in This
	Very Existence, “The Meanings of Sound,
	Word, and Reality.” Available in EC Manual.
	Session 11
	Dōgen, “Genjōkōan,” in The Heart of Dōgen’s
	Shōbōgenzō,
	Session 12
	Session 5
	Dōgen, “Uji,” in The Heart of Dōgen’s Shōbōgenzō,
	Sei Shōnagon, The Pillow Book, translated by
	Meredith McKinney (Penguin Classics),
	sections 1-100, pp. 3-113
	Session 13
	Session 6
	The Pillow Book, sections 101-206, 243, 258,
	273, pp. 113-190
	Session 7
	Kamo no Chōmei, “Hōjōki or the Record of the
	Ten-Foot Square Hut”
	Kenkō and Chōmei: Essays in Idleness and Hōjōki,
	translated by Meredith McKinney (Penguin)
	OR
	Four Huts: Asian Writings on the Simple Life,
	translated by Burton Watson (Shambhala)
	Kenkō, sections 1 – 38, 43 – 49, 52, 53, 58 –
	60, 66. In Kenkō and Chōmei: Essays in Idleness
	and Hōjōki, translated by Meredith McKinney
	(Penguin)
	OR
	Essays in Idleness, translated by Donald Keene
	(Columbia University Press)
	Session 14
	Kenkō, sections 69 – 75, 81 – 85, 89, 92, 97 –
	98, 104 – 122, 127 – 130, 133, 137, 154, 162,
	166, 184, 188, 190,191, 235 – 237.
	P a g e |7EASTERN CLASSICS
	SEMINAR
	SUMMER - SEMINAR
	(Continued)
	Session 15Session 16
	Bashō, “Journey of Bleached Bones in a
	Field,” “Kashima Journal,” “Knapsack
	Notebook, “Sarashina Journal” in Bashō’s
	Journey: The Literary Prose of Matsuo Bashō,
	translated by David Landis Barnhill (SUNY
	Press)Bashō, “The Narrow Road to the Deep North.”
	in Bashō’s Journey: The Literary Prose of
	Matsuo Bashō, translated by David Landis
	Barnhill (SUNY Press)
...
\begin{multicols}{2}
	...
	\printHeaderWithSessionAndDate{fall-precept1-start}
...
\begin{multicols}{2}
	...
	\printHeaderWithSessionAndDate{fall-precept2-start}
...
\begin{multicols}{2}
	...
	\printHeaderWithSessionAndDate{summer-precept-start}
